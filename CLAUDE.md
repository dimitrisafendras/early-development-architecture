...
- **Bullets** — one list marker, `BulletList` (`src/components/BulletList.tsx`). The app had three: a real dot span, a typed `•` glyph sitting on the text baseline, and the browser's `list-disc` whose size, colour and indent are the user agent's. `dotClassName` is for a list whose colour is **semantic** (the hunger vs fullness cues on `/wiki/feeding`); everything else takes the accent.
