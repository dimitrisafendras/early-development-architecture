...
- `/sleep` — the sleep log: start/stop for a sleep in progress, or both times at once for one already finished
...
**A sleep is an interval; a feed is an event.** That is the whole shape of `/sleep` and why it carries a running state the feed log does not: a feed is stamped at a moment, a sleep has a start you know when it happens and an end you only learn later. Both ways in are first-class — start it at bedtime and stop it in the morning, or log the finished night with both times, which is what you actually do for a night nobody was awake to time. **A sleep belongs to the day it *started*** (`useSleepLog`): a night running 21:00–06:30 counted against the calendar day each half falls in would report every family as sleeping half as much as they do, twice. Night vs nap is **derived from the clock and never stored** — it is a caption, and every number on the page counts minutes, so if the rule is wrong it is wrong in the label and nowhere else.

**Logs are scoped to one baby.** Reads used to ignore `baby_id` while writes carried it, so a second child's sessions and feeds counted as the first's. `listSessionsSince` / `findOpenSession` / `listFeedsSince` / `listSleepsSince` / `findOpenSleep` all take a baby now, and the local lists filter the same way — **including `useReportData`**, which omitted it longest: with no id the server query means *rows belonging to nobody*, so every signed-in household's `/export` report stated zero feeds and zero tummy time on the one page meant to be handed to a doctor. Rows with no `baby_id` are a legacy bucket that drains, since every new row is stamped. Both hooks ignore stale responses — `babyId` arrives asynchronously, so an unscoped load is always in flight beside the scoped one and used to win.
...
**A logger belongs to an activity kind, or it is unreachable.** The Day dashboard's tool zone is this app's central promise: at 12:30 the moment card shows the feed, and the feed form is *inside it* — you never navigate to log the thing you are doing. That promise is kept by two mappings that must agree, and both are exhaustive by type so neither can be forgotten:

- **`dayActivityMeta[kind].tool`** (`src/components/dayActivity.tsx`) — the link out to the full widget page, or `null`.
- **`momentWidgets[kind]`** (`src/pages/Day.tsx`) — the widget rendered *in* the moment card, typed `Record<DayActivity, … | null>`, so adding a `DayActivity` fails the build until you say what logs it and `null` is a decision rather than an omission.

So: **shipping a logging page means wiring it to every activity kind it logs, in the same change.** A page that exists but has no kind pointing at it is a page the caregiver can only reach by leaving the moment it belongs to — which is exactly how the sleep log shipped, as a route the dashboard had no branch for. `sleep` points at `/sleep`, `feed` and `meal` at `/feed`, `tummy` and `active` at `/tracker`; `play`, `care` and `wind` are `null` because nothing logs them, and the topic's guidance stands in. **Wind-down is `null` on purpose**: it carried the sleep console because it ends in a sleep, but that sleep has its own moment half an hour later, and a timer started here would record the bath-and-lullaby half hour as sleep — the card announced "0min slept today" over a routine that is not sleep at all.

**Every moment card is tool *then* tips, for all nine kinds.** `momentTips[kind]` (`src/pages/Day.tsx`) is the third exhaustive `Record<DayActivity, …>` — a heading and a set of title-then-sentence points, rendered by `MomentTips` under the widget with a rule above it, by `MomentTool` rather than by each widget. The safe-sleep rules used to hang off `SleepWidget`, so the app's one piece of "while you are doing this, here is what matters" reached exactly one of nine activities and the other eight ended at a logger or a one-line blurb. **The tips add no strings**: every list is already written, sourced and translated for the Wiki topic the activity maps to (`dayActivityMeta[kind].wiki`) — `feeding.cues*` for a feed, `soothing.practices` for wind-down, `bathing.safety` for care. Authoring dashboard-only copy here is how the two start to disagree, and `design.spec.ts` asserts the headings verbatim to catch it. `safety: true` swaps the lamp for the shield, and is for directives, not advice.

**A widget that appears on both a page and the dashboard is one component, not two.** `/feed` and `/daily` share `FeedProgress` + `AddFeedForm`; `/tracker` and `/daily` share `TummyConsole`; `/sleep` and `/daily` share `SleepConsole` + `AddSleepForm`. Each takes a `compact` prop for the dashboard, and that prop is a *density* switch only — smaller type, a shorter caption. If the two screens would say different things, that belongs in the caller. The tummy pair spent a while as two different instruments (a session bar on the page, a progress ring with its own clock and labels on the dashboard), which is exactly the drift this rule exists to stop.

Shared daily logic is hook-first: `useDailyChecklist` (checklist + streak + sync), `useTummyTracker`, `useSleepLog`, `useFeedLog`, `useBabies`, `useHousehold`, `useBabyAge` (age-band highlighting). Both the `/daily` widgets and the standalone sections/pages reuse these — don't duplicate the state.
...
| Path | Purpose |
|---|---|
| `src/pages/` | Route components (`Day.tsx` = home split view, `Wiki.tsx` / `WikiTopic.tsx`, `Tracker.tsx`, `FeedLog.tsx`, `SleepLog.tsx`, `Baby.tsx`, `Family.tsx`, `Export.tsx`, `Auth.tsx`, `DesignSystem.tsx`) |
...
| `src/components/` | Shared app components — `FactList` (a titled list of points inside one surface — see `src/sections/`), `Layout` (app shell), `SideNav` (desktop rail) / `NavBar` (mobile bar) / `BottomNav`, `SettingsMenu`, `AuroraBackground`, `PageFrame`, `WidgetPage`, `SectionHeader`, `StatTile`, `AgeBadge`, `ProgressRing`, `ActivityLink` (every link out of a moment — the timeline's Wiki chip and the dashboard's tool links are one component in the activity's own hue), `ErrorBoundary` (mounted above the router in `main.tsx` — an SPA render error otherwise leaves a blank page that still returns 200), `dayActivity`, `charts.tsx` |
...
| `src/sections/` | The Wiki topic sections (registry-driven, rendered on the Wiki topic pages). **A card is for an object you compare — an age band, a stat. A list of points is a `FactList` inside one card**: half of these were grids of four-to-six identical mini-cards, each holding an icon chip, a four-word title and one sentence, which made a six-point list read as six things to weigh and ran the interaction topic to eight phone viewports. Colour here is semantic only (`statusTone` — success/warning/danger); the `scheduleTone` table that gave age bands and day blocks a hue each is gone, because nothing let a reader decode it and six at once hid the colours that do mean something |
...
**Every logging page is a widget page** — any page whose job is "check one thing, then record one thing": `/tracker`, `/feed`, `/sleep`, `/baby`, and any new one. (`/daily` and `/routine` are dashboards, not widget pages.) A widget page always reads top-to-bottom in three tiers:
...
- **Bullets** — one list marker, `BulletList` (`src/components/BulletList.tsx`). The app had three: a real dot span, a typed `•` glyph sitting on the text baseline, and the browser's `list-disc` whose size, colour and indent are the user agent's. `dotClassName` is for a list whose colour **carries meaning** — semantic (the hunger vs fullness cues on `/wiki/feeding`) or an identity the rest of the surface already wears (a moment card's tips take that activity's hue from `dayActivityMeta`, as its mark, rail, halo and links all do; `--primary` there was the one element tracking the *palette* on a card that tracks the *activity*). Everything else takes the accent.
- **A field wrapper is `flex flex-col gap-*`, never `space-y-*`.** base-ui renders a popover's focus guards as `position: fixed` spans *beside* the trigger, inside the field's own wrapper. Tailwind's `space-y-*` keys off `:last-child`, so the moment those guards appeared the real control stopped being the last child, picked up a 6px margin, and the field grew — pushing every field after it down the row while the caregiver was mid-tap. `gap` is only drawn between *flex items*, and an out-of-flow span is not one. This applies to any wrapper holding a `DatePicker` / `TimePicker` / `DateTimePicker` / `Popover` / `Select`; `design.spec.ts` opens one and asserts the row's `offsetHeight` is unchanged (layout-relative, because opening a popover scrolls the page on a phone).
- **Every history is `HistoryList`** (`src/components/HistoryList.tsx`) — newest first, grouped by day with Today / Yesterday / the date, inside one capped `GlassScrollArea`. `/tracker` had this; `/feed` and `/sleep` each listed *today only* and grew with it, so a feed at 23:50 or a night that ran to 06:00 left the page at midnight and a newborn's eight feeds pushed the chart and the guidance off the bottom of a phone. `at` is the instant that decides the day — for a sleep the **start**, so a night files under the day it began. The rows stay the caller's: three different facts with three different edit forms, sharing only the shape of the list around them.
- **Selection must not animate geometry.** Selecting a step on the Day timeline grows the cell, its padding and its mark *while* the timeline smooth-scrolls to centre it — so anything geometric in a `transition-*` list puts a moving target under that scroll, and the mark (which has never been transitioned) snaps while the card glides. Colour, border and shadow animate; width, padding, margin and font-size do not. `design.spec.ts` asserts it on computed style, because headless Chromium does not animate `scrollIntoView` and cannot see the jank itself.
