      ...
      <div className="relative z-10 mx-auto w-full max-w-7xl px-6">
        ...
        <div className="flex flex-col items-start justify-between gap-8 lg:flex-row lg:items-center">
          ...
          <div className="dark flex flex-wrap gap-3">
            <GlassButton tone="primary" size="lg" render={<Link to={topicPath('action-items')} />}>
              <CheckSquare />
              {t.hero.ctaChecklist}
            </GlassButton>
            <GlassButton size="lg" className="text-white" render={<Link to={topicPath('daily-routine')} />}>
              <Clock />
              {t.hero.ctaSchedule}
            </GlassButton>
          </div>
          ...
          {heroMetrics.map((metric, i) => (
            <GlassSurface key={metric.label} radius={20} className="p-4 text-white">
              <div className="mb-1 text-[11px] font-medium uppercase tracking-wider text-white/70">
                {t.hero.metrics[i].label}
              </div>
              <div className="font-heading text-3xl font-semibold drop-shadow" style={{ color: metric.color }}>
                {metric.value}
              </div>
              <div className="mt-1 text-xs text-white/75">{t.hero.metrics[i].note}</div>
            </GlassSurface>
          ))}
        </div>
      </div>
