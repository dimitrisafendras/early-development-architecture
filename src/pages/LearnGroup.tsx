        ...
        <nav aria-label={t.hub.onThisPage} className="flex flex-wrap items-center gap-2">
          ...
          {topics.map((topic) => (
            <a
              key={topic.slug}
              href={`#${topic.slug}`}
              className="inline-flex min-h-11 items-center rounded-full bg-muted px-3.5 text-sm font-medium text-muted-foreground transition-colors hover:bg-accent hover:text-foreground sm:min-h-9"
            >
              {topic.label(t)}
            </a>
          ))}
