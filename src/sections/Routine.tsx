          ...
          return (
            <Card key={block.time} className="h-full">
              <CardContent className="flex h-full flex-col justify-between">
                <div>
                  {/* The window is a *label*, not an identity. Each of the six
                      blocks used to carry its own saturated hue from
                      `scheduleTone` — amber, emerald, sky, cyan, fuchsia,
                      indigo — which encoded nothing a reader could decode
                      (nothing says the evening block is fuchsia) and put six
                      accent families in one viewport. Muted, so the six read as
                      one series. */}
                  <Eyebrow as="h3" size="sm" tone="muted" className="mb-2">
                    {block.time}
                  </Eyebrow>
                ...
                {/* A rule and a line of text, not a tinted panel inside a
                    card. The tint was carrying the block's hue and nothing else,
                    so with the hue gone it was a second surface drawing a second
                    boundary to say "this sentence is the point" — which a border
                    and a weight already say. */}
